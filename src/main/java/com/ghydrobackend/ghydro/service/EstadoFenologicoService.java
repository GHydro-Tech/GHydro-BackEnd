...
public class EstadoFenologicoService {
    ...
    public EstadoFenologico buscarPorId(Long id) {
        return estadoRepository.findById(id)
                .orElseThrow(() -> new RegraDeNegocioException("Estado Fenológico não encontrado com o ID: " + id));
    }
    ...
    public void deletarEstadoFenologico(Long id) {
        if (!estadoRepository.existsById(id)) {
            throw new RegraDeNegocioException("Estado Fenológico não encontrado para exclusão.");
        }
    ...
    private void validarCamposObrigatorios(EstadoFenologico e) {
        ...
        if (e.getOrdemSequencia() == null) {
            throw new RegraDeNegocioException("A ordem da sequência é obrigatória.");
        }
        if (e.getDuracaoDias() == null) {
            throw new RegraDeNegocioException("A duração em dias é obrigatória.");
        }
    }

    private void validarValoresAgronomicos(EstadoFenologico e) {
        if (e.getOrdemSequencia() <= 0) {
            throw new RegraDeNegocioException("A ordem da sequência deve ser maior que zero.");
        }
        if (e.getDuracaoDias() <= 0) {
            throw new RegraDeNegocioException("A duração em dias deve ser maior que zero.");
        }
        if (e.getKCFase() != null && e.getKCFase() <= 0) {
            throw new RegraDeNegocioException("O coeficiente Kc deve ser maior que zero.");
        }
        if (e.getProfundidadeRaiz_cm() != null && e.getProfundidadeRaiz_cm() <= 0) {
            throw new RegraDeNegocioException("A profundidade da raiz deve ser maior que zero.");
        }
    }
    ...
    private void carregarCultura(EstadoFenologico e) {
        ...
        Cultura cultura = culturaRepository.findById(e.getCultura().getId())
                .orElseThrow(() -> new RegraDeNegocioException("A Cultura informada não existe."));
    ...
    private void validarDuplicidade(EstadoFenologico e, Long idAtualizacao) {
        ...
        if (idAtualizacao == null) {
            if (estadoRepository.existsByOrdemSequenciaAndCulturaId(e.getOrdemSequencia(), culturaId)) {
                throw new RegraDeNegocioException("Já existe uma fase com a ordem " + e.getOrdemSequencia() + " para esta cultura.");
            }
            if (estadoRepository.existsByNomeFaseAndCulturaId(e.getNomeFase(), culturaId)) {
                throw new RegraDeNegocioException("Já existe uma fase chamada '" + e.getNomeFase() + "' para esta cultura.");
            }
        }
        // Se for atualização
        else {
            if (estadoRepository.existsByOrdemSequenciaAndCulturaIdAndIdNot(e.getOrdemSequencia(), culturaId, idAtualizacao)) {
                throw new RegraDeNegocioException("Já existe outra fase com a ordem " + e.getOrdemSequencia() + " para esta cultura.");
            }
            if (estadoRepository.existsByNomeFaseAndCulturaIdAndIdNot(e.getNomeFase(), culturaId, idAtualizacao)) {
                throw new RegraDeNegocioException("Já existe outra fase chamada '" + e.getNomeFase() + "' para esta cultura.");
            }
        }
    }
}